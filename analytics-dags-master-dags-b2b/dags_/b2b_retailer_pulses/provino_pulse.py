...
email = ['grigoriy.zorin@sbermarket.ru', 'provino18@mail.ru','elvira.vakhitova@sbermarket.ru']
...
select distinct
    ...
    from
        ...
        where(store_id = 50801) and delivery_window_id is not null
...
def pushed_df():
    ...
    SQL = """
            select distinct shipment_number from cdm.b2b__pulses_logs where process_name = 'provino_pulse'
            """
...
def send_mail_dag():
    ...
    if len(df) >0:
        ...
        log_df['process_name'] = 'provino_pulse'
        ...
        for i in np.unique(df['Номер заказа'].values):
            ...
            send_mail(to = email ,
                      subject = f"Заказ {i} Сбермаркет из ПроВино",
                      text = f"Заказ {i} Сбермаркет из ПроВино",
                      file_paths = [PATH]
                      )
    else:
...
with DAG(
        ...
        dag_id='provino_pulse',
        description='provino_pulse',
