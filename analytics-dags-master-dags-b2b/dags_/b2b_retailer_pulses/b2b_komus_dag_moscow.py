...
with DAG(
        ...
        dag_id='b2b_komus_pulse_moscow',
        description='b2b_komus_pulse_moscow',
    ...
    def send_mail_dag():
        ...
        df2 = df1[df1.store_id == 81049]
        df1 = df1[df1.store_id != 81049]

        df1 = df1.drop(columns = ['store_id'])
        ...
        df4 = df3[df3.store_id == 81049]
        df3 = df3[df3.store_id != 81049]

        df3 = df3.drop(columns = ['store_id'])
        df4 = df4.drop(columns = ['store_id'])

        curdt = date.today().strftime("%d_%m_%Y")

        PATH1 = f'b2b_Комус_СберМаркет_{curdt}.xlsx'
        PATH2 = f'b2b_Комус_СберМаркет_Магазин_2я_Луговая_{curdt}.xlsx'
        PATH3 = f'Грузополучатели_по_заказам_с_бумагой_{curdt}.xlsx'
        PATH4 = f'Грузополучатели_по_заказам_с_бумагой_Магазин_2я_Луговая_{curdt}.xlsx'
        ...
        email = ['ip@optovy.komus.net', 'anna.andreeva@sbermarket.ru', 'roman.gaylit@sbermarket.ru',
         'sia1eq@tvr.komus.net', 'faveq@tvr.komus.net','grigoriy.zorin@sbermarket.ru', 'aleksandra.pekhoto@sbermarket.ru',
         'klsvt@tvr.komus.net','baa1v@bony.komus.net']

        if (len(df1) > 0) & (len(df3) > 0):
            send_mail(to=email,
            subject="(Москва) B2B заказы Комус Сбермаркет",
            text="Информация по b2b заказам в Комусе через Сбермаркет г. Москва",
        ...
        if (len(df2) > 0) & (len(df4) > 0):
            send_mail(to=email,
            subject="(Москва, 2-я Луговая) B2B заказы Комус Сбермаркет",
            text="Информация по b2b заказам в Комусе через Сбермаркет г. Москва, магазин 2-я Луговая",
            file_paths=[PATH2,PATH4])
        elif (len(df2) > 0):
            send_mail(to=email,
            subject="(Москва, 2-я Луговая) B2B заказы Комус Сбермаркет",
            text="Информация по b2b заказам в Комусе через Сбермаркет г. Москва, магазин 2-я Луговая",
            ...
            logging.info('No Data Received (2nd store)')
