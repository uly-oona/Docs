...
email = ['misha.kand@gmail.com','grigoriy.zorin@sbermarket.ru']
...
def kalina_orders():
    ...
    from
        ...
        where retailer_id = 24223 and delivery_window_id is not null
...
def pushed_df():
    ...
    SQL = """
            select distinct shipment_number from cdm.b2b__pulses_logs where process_name = 'kalina_pulse'
            """
    return ch_pd_read_sql_query(SQL, ch_conn_id=CH_ANALYTICS__MARKETING_B2B__CONN_ID)
...
def send_mail_dag():
    ...
    if len(df) >0:
        ...
        log_df['process_name'] = 'kalina_pulse'
        ...
        for i in np.unique(df['Номер заказа'].values):
            ...
            send_mail(to = email ,
                      subject = f"Заказ {i} СберМаркет ритейлер Калина",
                      text = f"Заказ {i} СберМаркет ритейлер Калина",
                      file_paths = [PATH]
                      )
    else:
